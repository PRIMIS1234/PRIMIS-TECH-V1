...
const text = m.body.slice(prefix.length + cmd.length).trim();
  ...
  if (validCommands.includes(cmd)) {
    ...
    try {
      await m.React('🕘');
      await m.reply('A moment, *PRIMIS-TECH-V1* is generating your lyrics request...');
      ...
      if (result && result.lyrics) {
        ...
        let buttons = [{
          ...
          },
          {
            name: "cta_url",
            buttonParamsJson: JSON.stringify({
              display_text: "sʜᴏᴡ 💜 ғᴏʀ PRIMIS-TECH-V1",
              url: `https://whatsapp.com/channel/0029Vb5fyMc8aKvSYHiIy11g`
            })
          },
          {
            name: "quick_reply",
